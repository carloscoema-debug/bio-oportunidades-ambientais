...
Deno.serve(async (req) => {
  ...
  const { data: vagas, count } = await svc
    .from("vagas")
    .select("id, titulo, empresa_orgao, data_expiracao_calculada", { count: "exact" })
    .eq("status", "aprovada")
  ...
  // `count` é o total real (antes do limit de prévia) — o `.limit(12)` só corta a
  // lista exibida no e-mail; sem isso o assunto sempre dizia "12" mesmo havendo mais.
  const lista = vagas ?? [];
  const total = count ?? lista.length;
  if (total < MIN_VAGAS) {
    return json({ ok: true, pulou: true, motivo: "poucas_vagas", vagas: total });
  }

  const itens = lista
    .map((v) => `<li style="margin:0 0 6px 0">${esc(v.titulo)}${v.empresa_orgao ? ` <span style="color:#8A968C">— ${esc(v.empresa_orgao)}</span>` : ""}</li>`)
    .join("");
  const nota = total > lista.length ? ` — mostrando as ${lista.length} mais relevantes abaixo` : "";
  ...
  const html = `<div style="font-family:system-ui,Arial,sans-serif;max-width:560px;margin:auto;color:#1B2A21">
    ...
    <p style="font-size:14px;color:#5B6B60;margin:0 0 14px">Há <strong>${total}</strong> vaga(s) nova(s) sugerida(s)${nota}. Revise e envie quando quiser — nada é disparado automaticamente.</p>
  ...
  const resp = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: remetente,
      to: [para],
      subject: `BIO · newsletter pronta para revisar (${total} vagas)`,
      html,
    }),
  });

  return json({ ok: resp.ok, avisou: resp.ok, vagas: total, para });
});
